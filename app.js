...
function initRealtimeListeners() {
    database.ref('rooms').on('value', (snapshot) => {
        ...
        if (selectedDate) refreshRoomStatus(roomsData[selectedDate]);
    });
...
function renderCalendar() {
    database.ref('rooms').once('value', (snapshot) => {
        updateCalendarWithData(snapshot.val() || {});
    });
}
...
function refreshRoomStatus(dayData) {
    ...
    const data = dayData || { room3:{gender:'none',count:0}, room4:{gender:'none',count:0}, room6:{gender:'none',count:0} };
...
async function addBoardContent() {
    ...
    alert('비밀글이 등록되었습니다.');
    document.getElementById('board-author').value = ''; document.getElementById('board-content').value = '';
}
...
function renderBoard(posts) {
    ...
    Object.keys(posts).reverse().forEach(k => {
        ...
        item.innerHTML = `<div class="board-item-title" onclick="this.nextElementSibling.classList.toggle('active')"><span>🔒 비밀글 (${p.author})</span><span>${p.date}</span></div>
            <div class="board-item-content">${isAdmin ? `[비번:${p.pw}]<br>${p.content}<br><button class="btn-secondary" onclick="deletePost('${k}')">삭제</button>` : 
            `<input type="password" id="pw-${k}" style="width:80px;"><button class="btn-secondary" onclick="checkPw('${k}','${p.pw}')">확인</button><div id="tx-${k}" style="display:none; margin-top:10px;">${p.content}</div>`}</div>`;
...
// 저장 기능들
async function saveRoomGender(room) { if(!isAdmin) return; await database.ref(`rooms/${selectedDate}/${room}`).set({ gender: document.getElementById(`admin-select-${room}`).value, count: parseInt(document.getElementById(`admin-capa-${room}`).value) }); alert('저장됨'); }
