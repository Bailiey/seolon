...
function initRealtimeListeners() {
    ...
    database.ref('lp_schedule').on('value', (snap) => {
        ...
        if(document.getElementById('lp-morning-text')) document.getElementById('lp-morning-text').innerText = val.morning;
        if(document.getElementById('lp-evening-text')) document.getElementById('lp-evening-text').innerText = val.evening;
    });
    database.ref('guide').on('value', (snap) => {
        const val = snap.val() || "내용을 입력해주세요.";
        if(document.getElementById('guide-display-content')) document.getElementById('guide-display-content').innerText = val;
    });
...
function refreshRoomStatus(data) {
    ...
    ['room3','room4','room6'].forEach(r => {
        ...
        if(capa) {
            let html = `인원: ${rd.count} / ${max}`;
            if(rd.count >= max) html += ` <span class="full-label" style="color:#c05a5a; font-weight:400; margin-left:8px;">Full</span>`;
            capa.innerHTML = html;
        }
        if(isAdmin) {
            if(document.getElementById(`admin-select-${r}`)) document.getElementById(`admin-select-${r}`).value = rd.gender;
            if(document.getElementById(`admin-capa-${r}`)) document.getElementById(`admin-capa-${r}`).value = rd.count;
        }
    });
}
...
/* 게시판 로직 */
let currentPage = 1; const postsPerPage = 5;
async function addBoardContent() {
    ...
    await database.ref('posts').push({ author, pw, content, date: new Date().toLocaleDateString(), reply: "" });
    alert('이야기가 등록되었습니다.');
...
async function deletePost(key) {
    if(confirm('정말 삭제하시겠습니까?')) { await database.ref(`posts/${key}`).remove(); alert('삭제되었습니다.'); }
}
...
function renderBoard(posts) {
    ...
    list.innerHTML = ''; if (!posts) { list.innerHTML = '<p style="text-align:center; padding:20px;">글이 없습니다.</p>'; return; }
    const postKeys = Object.keys(posts).reverse();
    const paginatedKeys = postKeys.slice((currentPage - 1) * postsPerPage, currentPage * postsPerPage);
    paginatedKeys.forEach(key => {
        const p = posts[key]; const item = document.createElement('div'); item.className = 'board-item';
        const body = isAdmin ? `<div class="admin-view-area"><div style="color:var(--accent-color); margin-bottom:10px;">[비번: ${p.pw}]</div><div style="white-space:pre-wrap; margin-bottom:15px;">${p.content}</div><textarea id="reply-input-${key}" style="width:100%; background:#111; color:#fff; border:1px solid #444; padding:10px;">${p.reply || ''}</textarea><div style="margin-top:10px; display:flex; gap:10px;"><button class="btn-primary" onclick="saveReply('${key}')">답변저장</button><button class="btn-secondary" onclick="deletePost('${key}')">삭제</button></div></div>` :
        `<div id="pw-area-${key}"><input type="password" id="pw-${key}" placeholder="비밀번호" style="width:100px; background:#111; color:#fff; border:1px solid #333; padding:5px;"><button class="btn-secondary" onclick="checkPw('${key}', '${p.pw}')">확인</button></div><div id="tx-${key}" style="display:none; margin-top:10px;"><div style="white-space:pre-wrap; margin-bottom:15px;">${p.content}</div>${p.reply ? `<div style="background:#222; padding:15px; border-left:3px solid var(--accent-color);"><strong style="color:var(--accent-color);">서론 사장님 답변</strong><div>${p.reply}</div></div>` : ''}</div>`;
        item.innerHTML = `<div class="board-item-title" onclick="this.nextElementSibling.classList.toggle('active')"><span>🔒 비밀글 (${p.author}) ${p.reply ? '[답변완료]' : ''}</span><span>${p.date}</span></div><div class="board-item-content">${body}</div>`;
        list.appendChild(item);
    });
    renderPagination(postKeys.length);
}
function renderPagination(total) {
    const list = document.getElementById('board-list'); const totalPages = Math.ceil(total / postsPerPage); if(totalPages <= 1) return;
    const nav = document.createElement('div'); nav.style = "display:flex; justify-content:center; gap:10px; margin-top:25px;";
    for (let i = 1; i <= totalPages; i++) {
        const btn = document.createElement('span'); btn.innerText = i; btn.style = `cursor:pointer; padding:5px 12px; border:1px solid ${i === currentPage ? 'var(--accent-color)' : '#444'}; color:#fff; border-radius:4px;`;
...
async function saveLP() { await database.ref('lp_schedule').set({ morning: document.getElementById('input-lp-morning').value, evening: document.getElementById('input-lp-evening').value }); alert('LP 저장됨'); }
async function saveGuideParams() { await database.ref('guide').set(document.getElementById('admin-guide-textarea').value); alert('안내사항 저장됨'); }
